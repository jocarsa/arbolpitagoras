...
def draw_branch(x1, y1, angle, depth, thickness, img, out, max_depth, root_color, tip_color, segments=5, leaf_size_range=(5, 10), angle_variation=30):
    ...
    if depth > 0 and frames_written < total_frames:  # Stop if we reach the frame limit
        ...
        for i in range(segments):
            ...
            # Draw the shadow slightly offset from the actual line segment
            shadow_start = (x1 + shadow_offset, y1 + shadow_offset)
            shadow_end = (x2 + shadow_offset, y2 + shadow_offset)
            cv2.line(img, shadow_start, shadow_end, shadow_color, int(current_thickness), lineType=cv2.LINE_AA)
            overlay = img.copy()
            cv2.addWeighted(overlay, shadow_alpha, img, 1 - shadow_alpha, 0, img)

            # Draw the actual branch segment on top of the shadow
        ...
        if depth == 1:
            ...
            # Draw leaf shadow slightly offset from the actual leaf
            shadow_leaf_center = (x1 + shadow_offset, y1 + shadow_offset)
            cv2.ellipse(img, shadow_leaf_center, (leaf_width, leaf_height), angle + 90, 0, 360, shadow_color, -1)
            overlay = img.copy()
            cv2.addWeighted(overlay, shadow_alpha, img, 1 - shadow_alpha, 0, img)

            # Draw the actual leaf on top of the shadow
...
print(f"The 1-minute fractal tree animation with random leaf sizes, branch angles, and shadow effects has been saved as '{filename}' in the 'video' folder.")
