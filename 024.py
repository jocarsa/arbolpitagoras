...
def draw_branch(x1, y1, angle, depth, thickness, img, out, max_depth, root_color, tip_color, segments=5, leaf_size_range=(5, 10)):
    ...
    if depth > 0 and frames_written < total_frames:  # Stop if we reach the frame limit
        ...
        total_length = depth * random.randint(10, 20)
        ...
        if depth == 1:
            ...
            cv2.ellipse(img, (x1, y1), (leaf_width, leaf_height), angle + 90, 0, 360, branch_color, -1)

        # Recursive calls for child branches with adjusted angles
        draw_branch(x1, y1, angle - 30, depth - 1, target_thickness, img, out, max_depth, root_color, tip_color, segments, leaf_size_range)
        draw_branch(x1, y1, angle + 30, depth - 1, target_thickness, img, out, max_depth, root_color, tip_color, segments, leaf_size_range)
...
fps = 60
video_duration_seconds = 60*60  # 1 minute
...
while frames_written < total_frames:
    ...
    draw_branch(start_x, start_y, initial_angle, initial_depth, initial_thickness, img, out, initial_depth, root_color, tip_color, leaf_size_range=leaf_size_range)

# Release video writer and close display window
out.release()
cv2.destroyAllWindows()

print(f"The 1-minute fractal tree animation with random leaf sizes has been saved as '{filename}' in the 'video' folder.")
