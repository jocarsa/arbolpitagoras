...
def draw_branch(x1, y1, angle, depth, thickness, img, out, max_depth, root_color, tip_color, segments=5):
    ...
    if depth > 0 and frames_written < total_frames:  # Stop if we reach the frame limit
        ...
        if depth == 1:
            cv2.ellipse(img, (x1, y1), (5, 10), 0, 0, 360, branch_color, -1)  # Leaf with tip color
...
print(f"The 1-minute fractal tree animation with leaves has been saved as '{filename}' in the 'video' folder.")
