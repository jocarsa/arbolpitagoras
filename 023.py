...
def draw_branch(x1, y1, angle, depth, thickness, img, out, max_depth, root_color, tip_color, segments=5):
    ...
    if depth > 0 and frames_written < total_frames:  # Stop if we reach the frame limit
        ...
        # If depth is 1, draw a leaf (ellipse) at the branch tip
        if depth == 1:
            # Draw the leaf with angle rotated 90 degrees from the branch angle
            cv2.ellipse(img, (x1, y1), (5, 10), angle + 90, 0, 360, branch_color, -1)  # Leaf with tip color
...
print(f"The 1-minute fractal tree animation with longitudinal leaves has been saved as '{filename}' in the 'video' folder.")
